...
export type FlexColType =
  | "text" | "number" | "date" | "time" | "daterange" | "select" | "checkbox" | "file"
  | "people" | "tags" | "link" | "phone" | "email" | "location" | "rating" | "timer" | "vote";

export type FlexOption = { label: string; color: string };
export type FlexColumn = { id: string; name: string; type: FlexColType; options?: FlexOption[] };
export type FlexGroup = { id: string; name: string; color: string };
export type FlexRow = Record<string, string | boolean | undefined>;
export type FlexSheetData = { columns: FlexColumn[]; rows: FlexRow[]; groups?: FlexGroup[] };
  ...
  const renderCell = (col: FlexColumn, row: FlexRow) => {
    ...
    switch (col.type) {
        ...
        return (
          <div className="flex items-center justify-center gap-1 h-9 px-1">
            <label className="cursor-pointer text-teal-600 hover:text-teal-700" title="رفع ملف">
              ...
              <input type="file" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; const d = await fileToDataUrl(f); setCell(rid, col.id, `${f.name}|${d}`); e.target.value = ""; }} />
            </label>
            {du ? (
      ...
      case "select": {
        const o = (col.options ?? []).find((x) => x.label === raw);
        return (
          <select value={(raw as string) ?? ""} onChange={(e) => setCell(rid, col.id, e.target.value)} className={inputCls + " appearance-none font-semibold rounded"} style={raw ? { background: (o?.color ?? "#64748b") + "22", color: o?.color ?? "#334155" } : undefined}>
            <option value="">—</option>
